...
- (void)showAlertView
{
    ...
    if (_strAlertTitle != nil && _strAlertTitle.length > 0)
    {
        ...
        if (_bDestructive) {
            lbTitle.font = [UIFont preferredFontForTextStyle:UIFontTextStyleHeadline];
            lbTitle.textColor = DO_DESTRUCTIVE_TEXT_COLOR;
        } else {
            lbTitle.font = [UIFont preferredFontForTextStyle:UIFontTextStyleHeadline];
            lbTitle.textColor = DO_TITLE_COLOR;
        }
    ...
    if (_bDestructive) {
        lbBody.font = [UIFont preferredFontForTextStyle:UIFontTextStyleHeadline];
        lbBody.textColor = DO_DESTRUCTIVE_TEXT_COLOR;

    } else {
        if (_strAlertTitle.length > 0) {
            lbBody.font = [UIFont preferredFontForTextStyle:UIFontTextStyleBody];
        } else {
            lbBody.font = [UIFont preferredFontForTextStyle:UIFontTextStyleHeadline];
        }
        lbBody.textColor = DO_TEXT_COLOR;
    }
    ...
    if (_doNo != nil)
    {
        UIButton *btNo = [UIButton buttonWithType:UIButtonTypeSystem];
        //btNo.frame = CGRectMake(0, dHeight, _vAlert.frame.size.width / 2.0 - 1, 40);
        btNo.frame = CGRectMake(_vAlert.frame.size.width / 2.0 - 0.5, dHeight, _vAlert.frame.size.width / 2.0 - 1, 40);
        //btNo.backgroundColor = DO_BUTTON_BOX;
        btNo.tag = DO_NO_TAG;
        
        [btNo setTitle:NSLocalizedString(@"NO",) forState:UIControlStateNormal];
        //[btNo setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    ...
    if (_doYes != nil)
    {
        UIButton *btYes = [UIButton buttonWithType:UIButtonTypeSystem];
        if (_doNo == nil) {
            btYes.frame = CGRectMake(0, dHeight, _vAlert.frame.size.width, 40);
            [btYes setTitle:NSLocalizedString(@"OK",) forState:UIControlStateNormal];

        } else {
            btYes.frame = CGRectMake(0, dHeight, _vAlert.frame.size.width / 2.0 + 0.5, 40);
            [btYes setTitle:NSLocalizedString(@"YES",) forState:UIControlStateNormal];
            
            if (_bDestructive) {
                [btYes setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
            }
        }
